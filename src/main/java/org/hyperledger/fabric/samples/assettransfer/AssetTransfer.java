...
public final class AssetTransfer implements ContractInterface {
    ...
    /**
     * Duplicate a asset on the ledger and assign that to a new owner.
     *
     * @param ctx the transaction context
     * @param assetID the ID of the asset being duplicated
     * @param owner new owner of the duplicated asset
    ...
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String owner) {
        ...
        //asset object that being duplicated
        Asset asset = genson.deserialize(assetJSON, Asset.class);
        ...
        for (KeyValue result: results) {
            Asset assetX = genson.deserialize(result.getStringValue(), Asset.class);
            assetXId = assetX.getAssetID();
            assetXId = assetXId.replace("asset", "");
            try {
                intValue = Integer.parseInt(assetXId);
                if(maxId < intValue) {
                	maxId = intValue;
                }
        ...
        //create a new unique asset id for duplicated asset
        duplicatedAssetID = "asset"+ String.valueOf(maxId+1);
        
        //duplicate asset
        Asset duplicatedAsset = new Asset(duplicatedAssetID, asset.getColor(), asset.getSize(), owner, asset.getAppraisedValue());
